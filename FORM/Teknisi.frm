...
Private Sub tbh()
Teknisi_TU.LBLKODE = 1
Teknisi_TU.Show vbModal
End Sub

Private Sub ubh()
Teknisi_TU.LBLKODE = 2
lblpos = rs.AbsolutePosition
KODE = 2

Teknisi_TU.lblkdteknisi = rs!kdteknisi
Teknisi_TU.txtnmteknisi = rs!nmteknisi
Teknisi_TU.CMBStatus.Text = rs!Status
Teknisi_TU.ChkNA = rs!non_aktif
...
Private Sub all()
If txtcari = "" Then
sql = "select a.*,N_aktif = case when a.non_aktif=1 then 'X' else '' end  from teknisi a order by a.kdteknisi"
Else
sql = "select a.*,N_aktif = case when a.non_aktif=1 then 'X' else '' end  from teknisi a where " & kategori & " like '%" & txtcari & "%' order by a.kdteknisi"
End If
...
kategori = "a.nmteknisi"
ElseIf CMBCARI.ListIndex = 1 Then
kategori = "a.kdteknisi"
End If
