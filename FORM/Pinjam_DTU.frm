...
Begin VB.Form Pinjam_DTU 
   ...
   Begin VB.Image Image1 
      Height          =   3390
      Left            =   0
      Picture         =   "Pinjam_DTU.frx":E6F3
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "Pinjam_DTU"
...
sqlA = "select a.kdbarang,sum(a.unit) as UKeluar,b.kdpo from pinjam_d a left join pinjam b  on a.kdpinjam =b.kdpinjam where b.kdpo ='" & Pinjam_D.txtkdPO & "' group by a.kdbarang,b.kdpo"
...
sqlA1 = "select a.kdbarang,b.nmbarang,a.unit,isnull(c.Ukeluar,0) as Ukeluar,b.satuan,a.keterangan,a.kdpo_d from po_d a left join barang b " & vbCrLf & _
        "on a.kdbarang=b.kdbarang left join (" & sqlA & ") c on a.kdPO=c.kdPO and a.kdbarang=c.kdbarang where a.kdpo='" & Pinjam_D.txtkdPO & "' "
...
Barang_BR.lblkode = UCase("PINJAM_DTU")
...
sql1 = "select kdbarang,sum(U_beli + U_Rpinjam + U_Rsewa + M_unit - U_free - U_pinjam - U_sewa  - K_unit - repair) as unit," & vbCrLf & _
        ...
        "from RKP_stok where kdgudang='" & Pinjam_D.lblkdgudang & "' and tgl <= '" & Format(Pinjam_D.txttglpinjam, "yyyy/MM/dd") & "' and kdbarang='" & lblkdbarang & "' group by kdbarang"
...
On Error GoTo hell
    ...
    Else
         If lblkode = 1 Then
         con.Execute ("delete from PO_d where kdpo='" & Pinjam_D.txtkdPO & "' and kdbarang in (select kdbarang from barang where kdkategori='02') ")
         con.Execute ("delete from Pinjam_d where kdpinjam='" & Pinjam_D.lblKDPinjam & "' and kdbarang in (select kdbarang from barang where kdkategori='02') ")
         
         con.Execute ("insert into PO_D values ('" & lblkdbarang & "_" & Pinjam_D.txtkdPO & "','" & Pinjam_D.txtkdPO & "','" & lblkdbarang & "'," & CCur(txtunit) & ",'" & UCase(txtketerangan) & "')")
         con.Execute ("insert into Pinjam_D values ('" & lblkdbarang & "_" & Pinjam_D.lblKDPinjam & "','" & Pinjam_D.lblKDPinjam & "','" & lblkdbarang & "'," & CCur(txtunit) & "," & CCur(txtharga) & "," & CCur(lblrupiah) & ",'" & UCase(txtketerangan) & "')")
         
         SetTimer hWnd, NV_CLOSEMSGBOX, 1000, AddressOf TimerProc
         MsgBox "Data Telah di Tambah di POnya Juga", vbInformation, "Info !"
         Pinjam_D.TimerAll.Interval = 10
         ...
         Call cek_POminta
             ...
             sql = "update Pinjam_D set unit=" & CCur(txtunit) & ",harga=" & CCur(txtharga) & ",rupiah=" & CCur(lblrupiah) & ",keterangan='" & UCase(txtketerangan) & "' where kdPinjam_D='" & lblkdpinjam_d & "'"
             con.Execute (sql)
             SetTimer hWnd, NV_CLOSEMSGBOX, 1000, AddressOf TimerProc
