...
Begin VB.Form ATagih_TU 
   ...
   Begin VB.Image Image1 
      Height          =   3165
      Left            =   0
      Picture         =   "ATagih_TU.frx":A94C
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7260
   End
End
Attribute VB_Name = "ATagih_TU"
...
On Error GoTo hell
    ...
    Else
         ...
         If lblkode = 1 Then
             sql = "insert into area_tagih  values ('" & UCase(lblkdteknisi) & "','" & UCase(txtnmteknisi) & "')"
             con.Execute (sql)
             MsgBox "Data Telah Tersimpan", vbInformation, "Informasi !"
     
             ATagih.TimerAll.Interval = 10
         Else
             sql = "update are_tagih set nmarea='" & UCase(txtnmteknisi) & "' where kdarea='" & lblkdteknisi & "'"
             con.Execute (sql)
...
Private Sub lblkdteknisi_GotFocus()
SendKeys "{Home}+{End}"
End Sub

Private Sub lblkdteknisi_KeyDown(KeyCode As Integer, Shift As Integer)
If KeyCode = vbKeyDown Then
SendKeys vbTab
...
Private Sub lblkdteknisi_KeyPress(KeyAscii As Integer)
If KeyAscii = 27 Then
Unload Me
...
Private Sub lblkdteknisi_LostFocus()
lblkdteknisi = UCase(lblkdteknisi)
End Sub
