...
Begin VB.Form PO_DTU 
   ...
   Begin VB.Image Image1 
      Height          =   3165
      Left            =   0
      Picture         =   "PO_DTU.frx":E55F
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "PO_DTU"
...
sqlA1 = "select kdbarang,sum(U_beli + U_Rpinjam + U_Rsewa + M_unit - U_free - U_pinjam - U_sewa  - K_unit - repair) as U_stok," & vbCrLf & _
        "0 as U_PO from RKP_stok where kdgudang='" & PO_D.lblkdgudang & "' and kdbarang='" & lblkdbarang & "' group by kdbarang"

sqlA2 = "select a.kdbarang,0 as U_stok," & vbCrLf & _
        "sum(a.unit) as U_PO from PO_d a left join PO b on a.kdPO=b.kdPO where b.kdgudang='" & PO_D.lblkdgudang & "' and a.kdbarang='" & lblkdbarang & "' and b.kdkeluar ='' group by a.kdbarang"
...
Barang_BR.LBLKODE = UCase("PO_DTU")
...
If LBLKODE = 1 Then
    ...
    MsgBox "Stok di Gudang = " & rsST!U_stok & " ,PO Permintaan = " & rsST!U_PO & " ,Stok yg Tersedia = " & rsST!U_tersedia, vbInformation
    Else
    MsgBox "Tidak Ada Stok yg Tersedia", vbInformation, "Info !!"
    End If
Else
    If rsST.RecordCount <> 0 Then
    MsgBox "Stok di Gudang = " & rsST!U_stok & " ,PO Permintaan = " & CCur(rsST!U_PO) - CCur(lblunit_awal) & " ,Stok yg Tersedia = " & CCur(rsST!U_tersedia) + CCur(lblunit_awal), vbInformation
    Else
    MsgBox "Tidak Ada Stok yg Tersedia", vbInformation, "Info !!"
    End If
End If
...
'             If CCur(rsST!U_tersedia) < CCur(txtunit) Then
'                MsgBox "Tidak dapat disimpan , Stok yg tersedia = " & rsST!U_tersedia, vbCritical, "Error !"
'                Exit Sub
'             Else
                sql = "insert into PO_d values ('" & UCase(lblkdbarang) & "_" & UCase(PO_D.txtkdPO) & "','" & UCase(PO_D.txtkdPO) & "','" & UCase(lblkdbarang) & "'," & CCur(txtunit) & ",'" & UCase(txtketerangan) & "')"
                con.Execute (sql)
                SetTimer hWnd, NV_CLOSEMSGBOX, 1000, AddressOf TimerProc
...
'             End If
         Else
'             If (CCur(rsST!U_tersedia) + CCur(lblunit_awal)) < CCur(txtunit) Then
'                MsgBox "Tidak dapat disimpan , Stok yg tersedia = " & CCur(rsST!U_tersedia) + CCur(lblunit_awal)
'                Exit Sub
'             Else
                 sql = "update PO_d set unit=" & CCur(txtunit) & ",keterangan='" & UCase(txtketerangan) & "' where kdPO_d='" & lblkdPO_d & "'"
                 con.Execute (sql)
                 SetTimer hWnd, NV_CLOSEMSGBOX, 1000, AddressOf TimerProc
